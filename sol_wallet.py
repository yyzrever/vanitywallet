...
try:
    ...
    raise SystemExit(
        ...
        "Install with: pip install mnemonic solders base58"
    )
...
LEVELS: Dict[str, Level] = {
    "account": Level(
        ...
        lambda index: (index | HARDENED, HARDENED),
    ),
    "change": Level(
        "m/44'/501'/0'/i'",
        (44 | HARDENED, COIN | HARDENED, HARDENED),
        lambda index: (index | HARDENED,),
    ),
}
...
def validate_pattern(text: str, case_sensitive: bool, name: str) -> str:
    ...
    if not text:
        return text
    allowed = set(BASE58_ALPHABET) if case_sensitive else {char.lower() for char in BASE58_ALPHABET}
    ...
    raise SystemExit(f"{name} contains characters outside the base58 alphabet: {''.join(invalid)}")


def char_probability(character: str, case_sensitive: bool) -> float:
    if case_sensitive:
        return 1.0 / 58.0 if character in BASE58_ALPHABET else 0.0
    lowered = character.lower()
    matches = sum(1 for candidate in BASE58_ALPHABET if candidate.lower() == lowered)
    return matches / 58.0
...
def derive_child(key: bytes, chain: bytes, index: int) -> Tuple[bytes, bytes]:
...
class SolanaDeriver:
    def __init__(self, seed: bytes, level_key: str) -> None:
        level = LEVELS[level_key]
        self._tail = level.tail
        key, chain = master_key(seed)
        for step in level.prefix:
            key, chain = derive_child(key, chain, step)
        self._base = (key, chain)

    def _keypair(self, index: int) -> Keypair:
        key, chain = self._base
        for step in self._tail(index):
            key, chain = derive_child(key, chain, step)
        return Keypair.from_seed(key)

    def probe(self, index: int) -> str:
        return str(self._keypair(index).pubkey())

    def reveal(self, index: int) -> Tuple[str, str]:
        keypair = self._keypair(index)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="VanityWallet Solana - vanity address search over a BIP39 seed phrase.",
    ...
    search.add_argument("--prefix", default=None, help="desired base58 prefix")
    search.add_argument("--suffix", default=None, help="desired base58 suffix")
    search.add_argument("--case-sensitive", dest="case_sensitive", action="store_true", default=None,
                        help="require an exact case match")
...
def search_worker(config: SearchConfig, worker_id: int, results: "MPQueue[Message]", stop: MPEvent) -> None:
    ...
    try:
        deriver = SolanaDeriver(config.seed, config.level)
        probe = deriver.probe
...
def run_lookup(args: argparse.Namespace, seed: bytes) -> None:
    ...
    deriver = SolanaDeriver(seed, level_key)
